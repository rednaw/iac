...
Open it at **https://monitoring.&lt;base_domain&gt;** (e.g. [https://monitoring.rednaw.nl](https://monitoring.rednaw.nl)). Log in with `openobserve_username@monitoring.local` and the password; both are in the secrets file (`secrets/infrastructure-secrets.yml`).
