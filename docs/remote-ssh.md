...
> [!IMPORTANT]
> Do not make manual changes on the server. All configuration is software defined. Use Remote-SSH for troubleshooting and inspection only — treat the server as read-only.
...
| OpenObserve | http://localhost:57800/ | **`openobserve_username@observe.local`**, password from **`secrets/infra.yml`** |
